...
const PLAN_DAYS: Record<string, number | null> = {
  ...
  lifetime: null,
  credits: null, // Créditos não expiram
};
...
export default function RevendedorLicencas() {
  ...
  const load = async () => {
    ...
    // Sincroniza status dos pedidos de créditos pendentes com o provedor antes de listar
    try {
...
// ... keep existing code
  ...
  const requestRefund = async (o: Order) => {
    ...
    if (!confirm("Solicitar reembolso proporcional dos créditos não enviados? O valor será creditado no seu saldo.")) return;
  ...
  return (
    <div className="relative min-h-screen space-y-6 overflow-hidden">
      ...
      <div className="relative space-y-6">
        <PageHeader
          title="Minhas Vendas"
          description="Histórico completo das chaves geradas e recargas de créditos da sua conta."
        />
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-medium text-zinc-300">
                            {o.product_type === "credits" ? `Créditos (${o.credit_amount})` : (LABEL[o.license_type] ?? o.license_type)}
                          </span>
                          {o.is_test && (
                                ...
                                <>
                                  {String(getEffectiveStatus(o)).toLowerCase() === "cancelado" ? (
                                    ...
                                    <DropdownMenuItem
                                      ...
                                      onClick={() => window.open(`https://revendovable.store/recarga/${getProviderOrderId(o) ?? o.id}`, "_blank", "noopener,noreferrer")}
                                    >
                          ...
                          {isCreditOrder(o) ? (
                            <>
                              {String(getEffectiveStatus(o)).toLowerCase() === "cancelado" ? (
                                ...
                                <DropdownMenuItem onClick={() => window.open(`https://revendovable.store/recarga/${getProviderOrderId(o) ?? o.id}`, "_blank")}>
                                  <ExternalLink className="mr-2 h-3.5 w-3.5" /> Abrir pedido
                                </DropdownMenuItem>
                              )}
                            </>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-bold text-primary">
                          {detailsTarget.product_type === "credits" ? `Créditos (${detailsTarget.credit_amount})` : (LABEL[detailsTarget.license_type] ?? detailsTarget.license_type)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">

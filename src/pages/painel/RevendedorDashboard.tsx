  ...
  const reload = useCallback(async (opts: { silent?: boolean } = {}) => {
    ...
    try {
      ...
      const [
        ...
        supabase.from("orders").select("id", { count: "exact", head: true }).eq("reseller_id", r.id).eq("is_test", false).in("status", ["failed", "falha", "erro"]),
        supabase.from("orders").select("id", { count: "exact", head: true }).eq("reseller_id", r.id).eq("is_test", false).in("status", ["refunded", "reembolsado", "estornado", "revoked", "canceled", "cancelled", "cancelado"]),
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-card via-card to-background">
        ...
        <div className="relative p-6 md:p-10">
          <div className="space-y-7 text-center md:text-left">
            ...
            <div>
              <h1 className="font-display text-3xl md:text-5xl font-black leading-[0.95] tracking-tighter">
                {t("dashboard.title")}
                <br />

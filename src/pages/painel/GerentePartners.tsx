...
export default function GerentePartners() {
  ...
  // Créditos: chave = credits_amount (number como string)
  ...
  const save = async () => {
    ...
    // Validação: apenas créditos não podem ficar abaixo do custo base
    for (const pkg of creditPackages) {
      const v = Number(creditDraft[pkg.credits_amount] ?? 0);
      const base = minCredit(pkg.credits_amount);
      if (v > 0 && base > 0 && v <= base) {
        toast.error(`${pkg.credits_amount} créditos: valor precisa ser maior que o custo base ${formatBRL(base)}`);
        return;
      }
    }
              ...
              <div className="space-y-4 min-w-0">
                {!selectedReseller ? (
                  ...
                  <>
                    ...
                    {/* Card: matriz de preços de créditos */}
                    <div className="rounded-3xl border border-border bg-card shadow-sm overflow-hidden">
                      <div className="px-4 sm:px-5 py-3 border-b border-border flex items-center gap-3">
                        <div className="h-6 w-1 bg-primary rounded-full" />
                        <h3 className="font-display text-sm font-bold tracking-tight flex items-center gap-2">
                          <Coins className="h-4 w-4 text-primary" /> Preços de créditos
                        </h3>
                      </div>
                      {loadingPrices ? (
                              ...
                              >
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">créditos</span>
                                    <span

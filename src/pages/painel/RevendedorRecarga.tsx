...
export default function RevendedorRecargas() {
  ...
  return (
    <div className="relative min-h-screen bg-background pb-40 text-foreground overflow-x-hidden">
      ...
      {/* Minhas Recargas — histórico de PIX */}
      <section className="relative z-10 px-4 pb-12">
        <div className="mx-auto max-w-7xl">
          {(() => {
            const list = allRecharges ?? recentRecharges;
            const filtered = list.filter((r) => {
              if (reStatusFilter !== "all" && r.status !== reStatusFilter) return false;
              if (reSearch.trim()) {
            ...
            const statusMap: Record<string, { label: string; cls: string }> = {
              ...
              completed: { label: "Paga", cls: "bg-emerald-500/15 text-emerald-600 border-emerald-500/30" },
              pending: { label: "Pendente", cls: "bg-amber-500/15 text-amber-600 border-amber-500/30" },
              expired: { label: "Expirada", cls: "bg-zinc-500/15 text-zinc-400 border-zinc-500/30" },
              failed: { label: "Falhou", cls: "bg-rose-500/15 text-rose-600 border-rose-500/30" },
              canceled: { label: "Cancelada", cls: "bg-zinc-500/15 text-zinc-400 border-zinc-500/30" },
            };
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        Todas as recargas via PIX feitas na sua conta.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      onClick={() => (allRecharges ? loadAllRecharges() : resellerId && loadRecentRecharges(resellerId))}
                      disabled={loadingAllRecharges}
                      title="Atualizar"
                    >
                    ...
                    {allRecharges ? (
                      ...
                      <Button
                        size="sm"
                        className="h-9 text-xs font-bold bg-primary text-primary-foreground hover:bg-primary/90"
                        onClick={loadAllRecharges}
                        disabled={loadingAllRecharges}
                      >
                        {loadingAllRecharges ? (
                ...
                <div className="relative flex flex-wrap gap-2">
                  <div className="relative flex-1 min-w-[220px]">
                    ...
                    <Input
                      ...
                      placeholder="Buscar por ID, transação ou provedor…"
                      className="pl-9 h-9 text-xs"
                    />
                  </div>
                  <Select value={reStatusFilter} onValueChange={setReStatusFilter}>
                          ...
                          <div className="flex-1 min-w-[200px] space-y-1">
                            ...
                            {r.provider_transaction_id ? (
                              <div className="flex items-center gap-1.5">
                                <code className="font-mono text-[10px] text-muted-foreground truncate max-w-[300px]">
                                  {r.provider_transaction_id}
                                </code>
                                <button
                                  type="button"
                                  onClick={() => {
                                    navigator.clipboard?.writeText(r.provider_transaction_id!);

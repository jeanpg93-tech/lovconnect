...
-- Reprocessar o pagamento que ficou pendente por causa do bug
SELECT public.activate_reseller(
  '8cb9ccef-2c60-4264-bfa2-f487c9150937'::uuid,
  '5cd4edd6-f50d-414c-b946-cc2103f69722'::uuid,
  NULL
);

-- Notificar o revendedor que o painel foi ativado
INSERT INTO public.notifications (user_id, title, body, type)
SELECT r.user_id,
       'Painel ativado! 🎉',
       'Seu pagamento foi confirmado e seu painel de revendedor está totalmente liberado.',
       'activation_approved'
  FROM public.resellers r
 WHERE r.id = '8cb9ccef-2c60-4264-bfa2-f487c9150937';

-- Notificar gerentes via Telegram
INSERT INTO public.telegram_outbox (text)
SELECT '✅ <b>Painel ativado via PIX</b>' || E'\n' ||
       'Revendedor: ' || COALESCE(r.display_name, r.id::text) || E'\n' ||
       'Valor pago: R$ ' || to_char(p.amount_cents::numeric/100, 'FM999990.00')
  FROM public.activation_payments p
  JOIN public.resellers r ON r.id = p.reseller_id
 WHERE p.id = '5cd4edd6-f50d-414c-b946-cc2103f69722';

-- Notificar indicador (se houver)
INSERT INTO public.notifications (user_id, title, body, type)
SELECT ref_r.user_id,
       'Sua indicação ativou o painel! 🎉',
       COALESCE(referred.display_name, 'Seu indicado') || ' acabou de ativar o painel de revendedor. Você passará a receber comissão recorrente sobre as recargas dele(a).',
       'referral_activated'
  FROM public.reseller_referrals rr
  JOIN public.resellers ref_r   ON ref_r.id   = rr.referrer_reseller_id
  JOIN public.resellers referred ON referred.id = rr.referred_reseller_id
 WHERE rr.referred_reseller_id = '8cb9ccef-2c60-4264-bfa2-f487c9150937';
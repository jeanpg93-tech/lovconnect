CREATE OR REPLACE FUNCTION public.manager_list_claude_manual_orders(_limit integer DEFAULT 200)
 RETURNS TABLE(id uuid, plan_code text, code text, provider_api_key text, cost_cents integer, created_at timestamp with time zone, customer_name text, customer_whatsapp text, customer_email text, status text, cancelled_at timestamp with time zone, is_trial boolean, reseller_id uuid, reseller_display_name text, origin text)
 LANGUAGE plpgsql
  ...
  RETURN QUERY
    SELECT co.id,
     ...
     WHERE (co.is_manager_manual = true OR COALESCE(co.is_trial, false) = true)
       AND (co.code IS NOT NULL OR COALESCE(co.is_trial, false) = true)

...
-- 1) Settings: low balance alert toggle + threshold + cooldown timestamps
ALTER TABLE public.telegram_settings
  ADD COLUMN IF NOT EXISTS notify_low_balance boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS low_balance_threshold_cents bigint NOT NULL DEFAULT 5000,
  ADD COLUMN IF NOT EXISTS last_low_provider_alert_at timestamptz,
  ADD COLUMN IF NOT EXISTS last_low_gateway_alert_at timestamptz;

-- 2) Rich trigger: enrich balance_transactions notifications with detailed sale data
...
BEGIN
  ...
  IF NEW.kind = 'order_debit' THEN
    ...
    IF FOUND THEN
      ...
      IF _so.product_type = 'credits' OR _so.credit_amount IS NOT NULL THEN
        ...
        END IF;
      ELSE
        _dias := CASE _so.license_type
          WHEN 'pro_1d' THEN 1 WHEN 'pro_7d' THEN 7 WHEN 'pro_15d' THEN 15 WHEN 'pro_30d' THEN 30
          ELSE NULL END;
        _extra := _extra || E'\n' || '📦 Produto: Licença ' ||
  ...
  ELSIF NEW.kind = 'license_purchase' THEN
    ...
    SELECT o.*, rc.name AS cust_name, rc.whatsapp AS cust_wa
      INTO _ord

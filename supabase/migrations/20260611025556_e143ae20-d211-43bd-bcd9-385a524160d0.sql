...
BEGIN
  ...
  IF NEW.status = 'awaiting_confirm' THEN
    ...
    toggle_on := setts.notify_plan_awaiting_confirm;
  ELSIF NEW.status = 'active' THEN
    emoji := '✅'; title := 'Plano aprovado e ativado';
    toggle_on := setts.notify_plan_active;
  ...
  -- Append status to avoid colliding with previous status notifications for the same subscription.

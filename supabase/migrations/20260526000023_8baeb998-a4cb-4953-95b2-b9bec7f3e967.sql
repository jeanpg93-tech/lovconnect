...
BEGIN
  ...
  ELSIF NEW.kind = 'order_debit' THEN
    ...
    IF FOUND THEN
      ...
      IF _so.product_type = 'credits' OR _so.credit_amount IS NOT NULL THEN
        ...
        _extra := _extra || E'\n' || '🔗 Pedido no provedor: <code>' || COALESCE(_so.provider_order_id::text,'—') || '</code>';
        _extra := _extra || E'\n' || '📦 Produto: ' || _produto;
        _extra := _extra || E'\n' || '🚚 Entrega: ' || COALESCE(_so.delivery_type, '—');
        _extra := _extra || E'\n' || '✉️ Conta Lovable: ' || COALESCE(_so.lovable_email, '—');
        _extra := _extra || E'\n' || '🗂 Workspace: ' || COALESCE(_so.workspace_name, '—');
  ...
  ELSIF NEW.kind IN ('credit_purchase','credit_recharge_api') THEN
    ...
    IF FOUND THEN
      ...
      _extra := _extra || E'\n' || '🛠 Origem: ' || _origem;
    ELSE
      -- Fallback: credit_recharge_api antigo sem reference_id ligando a rcp

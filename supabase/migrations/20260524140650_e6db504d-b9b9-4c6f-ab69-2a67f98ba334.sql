CREATE OR REPLACE FUNCTION public.lookup_affiliate_code(_code text)
RETURNS jsonb
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  v_code record;
  v_owner_name text;
BEGIN
  SELECT id, code, owner_reseller_id, label, is_active, expires_at
    INTO v_code
    FROM public.affiliate_codes
   WHERE upper(code) = upper(trim(_code))
   LIMIT 1;

  IF NOT FOUND THEN
    RETURN jsonb_build_object('found', false);
  END IF;

  IF v_code.is_active = false OR (v_code.expires_at IS NOT NULL AND v_code.expires_at < now()) THEN
    RETURN jsonb_build_object('found', false, 'expired', true);
  END IF;

  IF v_code.owner_reseller_id IS NOT NULL THEN
    SELECT COALESCE(p.display_name, r.display_name)
      INTO v_owner_name
      FROM public.resellers r
      LEFT JOIN public.profiles p ON p.id = r.user_id
     WHERE r.id = v_code.owner_reseller_id
     LIMIT 1;
    RETURN jsonb_build_object(
      'found', true,
      'type', 'reseller',
      'owner_name', COALESCE(v_owner_name, 'Revendedor')
    );
  END IF;

  RETURN jsonb_build_object(
    'found', true,
    'type', 'campaign',
    'description', COALESCE(v_code.label, 'Campanha')
  );
END;
$function$;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: reseller } = await svc
      .from("resellers")
      .select("id,activation_status,billing_mode,subscription_blocked,subscription_sales_disabled,test_keys_per_day_override")
    ...
    // Captura IP/UA para auditoria de chaves teste geradas pelo painel
    ...
    // Trial: limite diário (override do revendedor ou tier padrão)
    if (isTrial) {
      let dailyLimit = 10;
